...
class SnekEnv(gym.Env):
    ...
    def __init__(self, reward_function):
        ...
        self.collision_apple = False
        self.collision_boundary = False
    ...
    def step(self, action):
        ...
        if collision_with_boundaries(self.snake_head) == 1 or collision_with_self(self.snake_position) == 1:
            ...
            cv2.imshow('a', self.img)
            self.collision_boundary = True
            self.done = True

        # self.euclidean_dist_to_apple = np.linalg.norm(np.array(self.snake_head) - np.array(self.apple_position))

        self.total_reward = self.reward_function({
            "snake_head": self.snake_head,
            "snake_position": self.snake_position,
            "apple_position": self.apple_position,
            "collision_apple": self.collision_apple,
